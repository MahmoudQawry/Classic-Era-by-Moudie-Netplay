...
class UniversalNetplayClient(
  ...
  fun setSessionReady(isReady:Boolean){
    if(isReady) transport?.send("netplay:universal-ready",JSONObject().put("system",config.system).put("fingerprint",config.fingerprint).put("coreVersion",config.coreVersion))
  }
