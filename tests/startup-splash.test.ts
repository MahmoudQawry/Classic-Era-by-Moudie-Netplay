...
describe("Android startup splash safeguards", () => {
  ...
  it("does not install Expo's native pre-draw splash gate that can retain the Android logo", () => {
    ...
    const workflow = readProjectFile(".github/workflows/android-build-reset.yml");
  ...
  it("plays only the official boot video after the lobby has mounted", () => {
    ...
    ["introSkip", "classic-era-ui-background.jpg", "VideoView", "useVideoPlayer"].forEach((label) => expect(intro).toContain(label));
