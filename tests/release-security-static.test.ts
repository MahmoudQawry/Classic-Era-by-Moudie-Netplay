...
describe("release security guards", () => {
  ...
  it("keeps runtime env examples free of credential-like demo values", () => {
    ...
    for (const name of ["LIVEKIT_API_SECRET", "REDIS_PASSWORD", "TURN_SHARED_SECRET"]) {
      expect(example).toMatch(new RegExp(`^${name}=replace_with_runtime_secretimport { existsSync, readFileSync } from "node:fs";
import { resolve } from "node:path";
import { describe, expect, it } from "vitest";
