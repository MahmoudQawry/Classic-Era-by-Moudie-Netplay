  ...
  return (
    <ScreenContainer className="px-5" edges={["top", "bottom", "left", "right"]}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Text style={styles.system}>{SYSTEM_LABEL[snapshot.room.system] ?? snapshot.room.system.toUpperCase()}</Text>
        ...
        {system === "nes" ? (
          ...
          <Pressable onPress={() => router.push({ pathname: "/psp/[roomId]", params: { roomId: String(roomId) } } as never)} style={({ pressed }) => [styles.playButton, styles.pressed && styles.pressed]}><Text style={styles.playText}>{t("rmEnterPsp")}</Text></Pressable>
        ) : nativeRoom ? (
          <Pressable onPress={() => router.push({ pathname: "/native/[system]/[roomId]", params: { system, roomId: String(roomId) } } as never)} style={({ pressed }) => [styles.playButton, pressed && styles.pressed]}><Text style={styles.playText}>{system === "n64" ? "ENTER NINTENDO 64 SETTINGS" : system === "ps2" ? "ENTER PLAYSTATION 2 SETTINGS" : t("rmEnterSega")}</Text></Pressable>
        ) : (
...
const styles = StyleSheet.create({
  content: { paddingVertical: 10, paddingBottom: 28 }, topRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" }, back: { paddingVertical: 7 }, backText: { color: "#9BAFC4", fontSize: 16, fontWeight: "700" },
