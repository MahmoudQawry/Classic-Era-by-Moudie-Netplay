...
function assertRateLimit(ip: string, action = "general") {
  const key = `${action}:${ip}`;
  const now = Date.now();
  const recent = (actionsByIp.get(key) ?? []).filter((at) => now - at < RATE_LIMIT_WINDOW_MS);
  if (recent.length >= RATE_LIMIT_MAX_ACTIONS) {
    throw new Error("عدد كبير من المحاولات. انتظر قليلاً ثم أعد المحاولة.");
  }
  recent.push(now);
  actionsByIp.set(key, recent);
  if (actionsByIp.size > 10_000) {
    for (const [entryKey, timestamps] of actionsByIp) {
      if (timestamps.every((at) => now - at >= RATE_LIMIT_WINDOW_MS)) actionsByIp.delete(entryKey);
    }
  }
}
...
export const appRouter = router({
  ...
  rooms: router({
    ...
    create: publicProcedure.input(z.object({
      ...
      assertRateLimit(ctx.req.ip ?? "unknown", "room-create");
    ...
    publicList: publicProcedure.input(z.object({ limit: z.number().int().min(1).max(50).default(30) })).query(async ({ input }) => {
      ...
      return rooms.map((room) => ({
        id: room.id, name: room.name, system: room.system, maxPlayers: room.maxPlayers,
        maxSpectators: roomCapacityFor(room.system as RoomSystem).maxSpectators, status: room.status,
        activePlayers: room.activePlayers, spectators: room.spectators, readyPlayers: room.readyPlayers,
    ...
    join: publicProcedure.input(z.object({
      ...
      assertRateLimit(ctx.req.ip ?? "unknown", "room-join");
      return seatMemberInRoom(await db.findRoomByCode(input.joinCode.toUpperCase()), input.displayName, input.joinAs);
    }),
    joinPublic: publicProcedure.input(z.object({
      roomId: z.number().int().positive(),
      displayName: z.string().trim().min(2).max(32),
      joinAs: z.enum(["player", "spectator"]).default("player"),
    })).mutation(async ({ input, ctx }) => {
      assertRateLimit(ctx.req.ip ?? "unknown", "room-join-public");
    ...
    snapshot: publicProcedure.input(z.object({
      roomId: z.number().int().positive(),
    ...
    mediaToken: publicProcedure.input(z.object({
      roomId: z.number().int().positive(),
      memberId: z.number().int().positive(),
      memberToken: z.string().min(20),
    })).mutation(async ({ input, ctx }) => {
      assertRateLimit(ctx.req.ip ?? "unknown", "media-token");
      const member = await db.getMemberByAccessToken(input.memberId, hashAccessToken(input.memberToken));
    ...
    setReady: publicProcedure.input(z.object({
      memberId: z.number().int().positive(),
      memberToken: z.string().min(20),
      isReady: z.boolean(),
      ...
      assertRateLimit(ctx.req.ip ?? "unknown", "room-ready");
    ...
    start: publicProcedure.input(z.object({
      ...
      assertRateLimit(ctx.req.ip ?? "unknown", "room-start");
      const snapshot = await db.getRoomSnapshot(input.roomId);
